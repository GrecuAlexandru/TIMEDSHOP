function searchPress() {
  if (
    ...
    index
      .search(searchInput.value, { page: currentSearchPage, hitsPerPage: 18 })
      .then((response) => {
        ...
        hits.forEach((qdoc) => {
          ...
          numberOfProducts = document.getElementById("productOutput").children.length;
          if(numberOfProducts<=6)
          {
            document.getElementById("fullProductsDiv").style.height = "150vh";
          }
          else
          {

            var marginTop = 150 + (~~((numberOfProducts-6)/3)*70);
            if(numberOfProducts % 3 != 0)
            {
              marginTop+=70;
            }
            document.getElementById("fullProductsDiv").style.height = marginTop+"vh"; 
          }
          //console.log(qdoc);
          //product item
          var product = document.createElement("a");
          product.setAttribute("href", "/product.html?id=" + qdoc.objectID);
          ...
          for (let i = 0; i < productColorsLength; i++) {
            ...
            productColorCircle.style.backgroundColor = qdoc.colors[i];
