...
if(sortBy=="newArrival")
{
  if(currentPage > 1)
  {
    ...
    if(whereSize.length-1 != 0)
    {
      ...
      {
        preQuery = query(collection(db, "women"), orderBy("price", "asc"), where("price", "<=", maxPrice),where("price", ">=", minPrice), orderBy("date", "desc"), where("type","in",whereType));
      ...
      {
        preQuery = query(collection(db, "women"), orderBy("price", "asc"), where("price", "<=", maxPrice),where("price", ">=", minPrice), orderBy("date", "desc")); 
        for(var i=1;i<=whereSize.length-1;i++)
        {
          preQuery = query(preQuery,where(whereSize[i],"==",true));
        }
      ...
      preQuery = query(collection(db, "women"), orderBy("price", "asc"), where("price", "<=", maxPrice),where("price", ">=", minPrice), orderBy("date", "desc"), where("type", "in", whereType), limit(resultsUntilStart));
    else
      preQuery = query(collection(db, "women"), orderBy("price", "asc"), where("price", "<=", maxPrice),where("price", ">=", minPrice), orderBy("date", "desc"), limit(resultsUntilStart));
