...
function openImage(imgsrc) {
  ...
  document.getElementById("fullScreenImageModal").style.visibility = "visible";
...
db.collection("women").doc(params.get("id")).get().then((doc) => {
    if(doc.exists)
    {
        //productTitle
        ...
        if (!isNaN(doc.data().priceBeforeSale)) {
            let productPriceBeforeSale = document.getElementById("productPriceBeforeSale");
            productPriceBeforeSale.innerText =
              "$" + doc.data().priceBeforeSale + ".00";
            let priceSaleAmount = document.getElementById("priceSaleAmount");
            productSaleAmount.innerText =
              (
                (Math.abs(doc.data().priceBeforeSale - doc.data().price) /
        ...
        for(let i = 0;i < colorsSize; i++)
        {
            ...
            colorBox.setAttribute("onclick",`selectColor("${colors[i].toString()}")`);
